...
def check_for_raid_button(screen_cv, region):
    ...
    found, conf = find_on_screen(get_template(HAVE_TO_CONNECT_IMG), screen_cv, region, threshold=CONFIDENCE_THRESHOLD)
    if found:
        print(f"[RAID] Найден рейд (HAVE_TO_CONNECT_IMG, conf={conf:.3f})")
        find_and_click(HAVE_TO_CONNECT_IMG, screen_cv, region, CONFIDENCE_THRESHOLD)
        return True

    # Проверка второй кнопки
    found, conf = find_on_screen(get_template(HAVE_TO_CONNECT_2_IMG), screen_cv, region, threshold=CONFIDENCE_THRESHOLD)
    if found:
        print(f"[RAID] Найден рейд (HAVE_TO_CONNECT_2_IMG, conf={conf:.3f})")
        find_and_click(HAVE_TO_CONNECT_2_IMG, screen_cv, region, CONFIDENCE_THRESHOLD)
        return True
